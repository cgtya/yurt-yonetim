...
public class Student extends User {
    ...
    public void makeBecayis() {
        ...
        try (Connection conn = DriverManager.getConnection(url, user, databasePassword)) {
            conn.setAutoCommit(false); // bu satır eğer birinci öğrencinin yurt bilgisi güncellenir ama öteki öğrencide hata alınırsa hata oluşmasını engellemek için önemlidir
            ...
            while (true) {
                ...
                try (PreparedStatement pstmt = conn.prepareStatement(sqlGetDorm)) {
                    ...
                    if (rs.next()) {
                        ...
                        System.out.println("Bu TC numarasına ait öğrenci bulunamadı. Lütfen tekrar deneyin.");
                    }
                }
            }
            ...
            if (dorm1.equalsIgnoreCase(dorm2)) {
                ...
                conn.rollback(); // veritabanı işlemi yapılmadıysa yine de garantiye alalım
                return;
            }
            ...
            System.out.println("Becayiş işlemi sırasında hata oluştu: " + e.getMessage());
        }
    }
